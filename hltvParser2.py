    ...
    for link in all_links:
        try:
            ...
            for map_num, h in enumerate(map_name1):
                ...
                team1_result = (
                    h.find("div", class_="results-left").find("div", class_="results-team-score").text.replace("-", "0")
                )
                team2_result = (
                    h.find("span", class_="results-right")
                    .find("div", class_="results-team-score")
                    .text.replace("-", "0")
                )
                head2head_team1_wins = (
                    m.find("div", class_="flexbox-column flexbox-center grow right-border")
                    .find("div", class_="bold")
                    .text
                )
                head2head_team2_wins = (
                    m.find("div", class_="flexbox-column flexbox-center grow left-border")
                    .find("div", class_="bold")
                    .text
                )
                ...
                try:
                    ...
                    team1_rounds_lose_mean_1month = (
                        np.mean(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_median_1month = (
                        np.median(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_mode_1month = (
                        stats.mode(team1_1month_map_results)[0][0] if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_max_1month = (
                        np.max(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_min_1month = (
                        np.min(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )

                    team1_diff_mean_1month = (
                        np.mean(team1_diff_with_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_diff_median_1month = (
                        np.median(team1_diff_with_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_diff_mode_1month = (
                        stats.mode(team1_diff_with_enemy_1month)[0][0] if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_diff_max_1month = (
                        np.max(team1_diff_with_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_diff_min_1month = (
                        np.min(team1_diff_with_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )

                    team1_rounds_vs_enemy_mean_1month = (
                        np.mean(team1_rounds_vs_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_median_1month = (
                        np.median(team1_rounds_vs_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_mode_1month = (
                        stats.mode(team1_rounds_vs_enemy_1month)[0][0] if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_max_1month = (
                        np.max(team1_rounds_vs_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_min_1month = (
                        np.min(team1_rounds_vs_enemy_1month) if len(team1_1month_map_results) != 0 else np.nan
                    )

                    team1_rounds_win_mean_1month = (
                        np.mean(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_win_median_1month = (
                        np.median(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_rounds_win_mode_1month = (
                        stats.mode(team1_1month_map_results)[0][0] if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_max_rounds_1month = (
                        np.max(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_min_rounds_1month = (
                        np.min(team1_1month_map_results) if len(team1_1month_map_results) != 0 else np.nan
                    ...
                    team1_1month_totals_mean = (
                        np.mean(team1_1month_map_totals) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_1month_totals_median = (
                        np.median(team1_1month_map_totals) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_1month_totals_mode = (
                        stats.mode(team1_1month_map_totals)[0][0] if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_1month_totals_max = (
                        np.max(team1_1month_map_totals) if len(team1_1month_map_results) != 0 else np.nan
                    )
                    team1_1month_totals_min = (
                        np.min(team1_1month_map_totals) if len(team1_1month_map_results) != 0 else np.nan
                    )
                except Exception as ex:
                    print(f"1 {ex}")
                ...
                try:
                    ...
                    team2_rounds_lose_mean_1month = (
                        np.mean(team2_1month_map_loses) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_median_1month = (
                        np.median(team2_1month_map_loses) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_mode_1month = (
                        stats.mode(team2_1month_map_loses)[0][0] if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_max_1month = (
                        np.max(team2_1month_map_loses) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_min_1month = (
                        np.min(team2_1month_map_loses) if len(team2_1month_map_results) != 0 else np.nan
                    )

                    team2_diff_mean_1month = (
                        np.mean(team2_diff_with_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_diff_median_1month = (
                        np.median(team2_diff_with_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_diff_mode_1month = (
                        stats.mode(team2_diff_with_enemy_1month)[0][0] if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_diff_max_1month = (
                        np.max(team2_diff_with_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_diff_min_1month = (
                        np.min(team2_diff_with_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )

                    team2_rounds_vs_enemy_mean_1month = (
                        np.mean(team2_rounds_vs_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_median_1month = (
                        np.median(team2_rounds_vs_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_mode_1month = (
                        stats.mode(team2_rounds_vs_enemy_1month)[0][0] if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_max_1month = (
                        np.max(team2_rounds_vs_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_min_1month = (
                        np.min(team2_rounds_vs_enemy_1month) if len(team2_1month_map_results) != 0 else np.nan
                    )

                    team2_rounds_win_mean_1month = (
                        np.mean(team2_1month_map_results) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_win_median_1month = (
                        np.median(team2_1month_map_results) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_rounds_win_mode_1month = (
                        stats.mode(team2_1month_map_results)[0][0] if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_max_rounds_1month = (
                        np.max(team2_1month_map_results) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_min_rounds_1month = (
                        np.min(team2_1month_map_results) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_1month_totals_mean = (
                        np.mean(team2_1month_map_totals) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_1month_totals_median = (
                        np.median(team2_1month_map_totals) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_1month_totals_mode = (
                        stats.mode(team2_1month_map_totals)[0][0] if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_1month_totals_max = (
                        np.max(team2_1month_map_totals) if len(team2_1month_map_results) != 0 else np.nan
                    )
                    team2_1month_totals_min = (
                        np.min(team2_1month_map_totals) if len(team2_1month_map_results) != 0 else np.nan
                    )
                except Exception as ex:
                ...
                try:
                    ...
                    team1_rounds_lose_mean_3month = (
                        np.mean(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_median_3month = (
                        np.median(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_mode_3month = (
                        stats.mode(team1_3month_map_results)[0][0] if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_max_3month = (
                        np.max(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_lose_min_3month = (
                        np.min(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )

                    team1_diff_mean_3month = (
                        np.mean(team1_diff_with_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_diff_median_3month = (
                        np.median(team1_diff_with_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_diff_mode_3month = (
                        stats.mode(team1_diff_with_enemy_3month)[0][0] if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_diff_max_3month = (
                        np.max(team1_diff_with_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_diff_min_3month = (
                        np.min(team1_diff_with_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )

                    team1_rounds_vs_enemy_mean_3month = (
                        np.mean(team1_rounds_vs_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_median_3month = (
                        np.median(team1_rounds_vs_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_mode_3month = (
                        stats.mode(team1_rounds_vs_enemy_3month)[0][0] if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_max_3month = (
                        np.max(team1_rounds_vs_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_vs_enemy_min_3month = (
                        np.min(team1_rounds_vs_enemy_3month) if len(team1_3month_map_results) != 0 else np.nan
                    )

                    team1_rounds_win_mean_3month = (
                        np.mean(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_win_median_3month = (
                        np.median(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_rounds_win_mode_3month = (
                        stats.mode(team1_3month_map_results)[0][0] if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_max_rounds_3month = (
                        np.max(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_min_rounds_3month = (
                        np.min(team1_3month_map_results) if len(team1_3month_map_results) != 0 else np.nan
                    )

                    team1_3month_totals_mean = (
                        np.mean(team1_3month_map_totals) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_3month_totals_median = (
                        np.median(team1_3month_map_totals) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_3month_totals_mode = (
                        stats.mode(team1_3month_map_totals)[0][0] if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_3month_totals_max = (
                        np.max(team1_3month_map_totals) if len(team1_3month_map_results) != 0 else np.nan
                    )
                    team1_3month_totals_min = (
                        np.min(team1_3month_map_totals) if len(team1_3month_map_results) != 0 else np.nan
                    )
                ...
                try:
                    ...
                    team2_rounds_lose_mean_3month = (
                        np.mean(team2_3month_map_loses) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_median_3month = (
                        np.median(team2_3month_map_loses) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_mode_3month = (
                        stats.mode(team2_3month_map_loses)[0][0] if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_max_3month = (
                        np.max(team2_3month_map_loses) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_lose_min_3month = (
                        np.min(team2_3month_map_loses) if len(team2_3month_map_results) != 0 else np.nan
                    )

                    team2_diff_mean_3month = (
                        np.mean(team2_diff_with_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_diff_median_3month = (
                        np.median(team2_diff_with_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_diff_mode_3month = (
                        stats.mode(team2_diff_with_enemy_3month)[0][0] if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_diff_max_3month = (
                        np.max(team2_diff_with_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_diff_min_3month = (
                        np.min(team2_diff_with_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )

                    team2_rounds_vs_enemy_mean_3month = (
                        np.mean(team2_rounds_vs_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_median_3month = (
                        np.median(team2_rounds_vs_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_mode_3month = (
                        stats.mode(team2_rounds_vs_enemy_3month)[0][0] if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_max_3month = (
                        np.max(team2_rounds_vs_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_vs_enemy_min_3month = (
                        np.min(team2_rounds_vs_enemy_3month) if len(team2_3month_map_results) != 0 else np.nan
                    )

                    team2_rounds_win_mean_3month = (
                        np.mean(team2_3month_map_results) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_win_median_3month = (
                        np.median(team2_3month_map_results) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_rounds_win_mode_3month = (
                        stats.mode(team2_3month_map_results)[0][0] if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_max_rounds_3month = (
                        np.max(team2_3month_map_results) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_min_rounds_3month = (
                        np.min(team2_3month_map_results) if len(team2_3month_map_results) != 0 else np.nan
                    )

                    team2_3month_totals_mean = (
                        np.mean(team2_3month_map_totals) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_3month_totals_median = (
                        np.median(team2_3month_map_totals) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_3month_totals_mode = (
                        stats.mode(team2_3month_map_totals)[0][0] if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_3month_totals_max = (
                        np.max(team2_3month_map_totals) if len(team2_3month_map_results) != 0 else np.nan
                    )
                    team2_3month_totals_min = (
                        np.min(team2_3month_map_totals) if len(team2_3month_map_results) != 0 else np.nan
                    )
